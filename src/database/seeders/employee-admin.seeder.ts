...
export class EmployeeAdminSeeder implements Seeder {
  ...
  async seed() {
    ...
    const employeeAdmins: Partial<Employee>[] = createdUsers.map(user => ({
      user,
      startDate: new Date(),
      department: Object.values(EmployeeDepartment).find(dept => `${dept}admin` === user.userName),
    }));

    await Promise.all(employeeAdmins.map(async employee => await this.employeeRepository.save(this.employeeRepository.create(employee))));
  }
