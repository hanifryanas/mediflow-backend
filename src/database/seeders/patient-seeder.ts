...
export class PatientSeeder implements Seeder {
  private generatedPatientUsers: Partial<User>[] = Array.from({ length: 50 }, () => {
  ...
  private generatedPatientInsurances: Partial<PatientInsurance>[] = Array.from({ length: 50 }, (): Partial<PatientInsurance> => {
    return {
      insuranceProvider: faker.helpers.arrayElement(Object.values(InsuranceProviderType)),
  ...
  async seed() {
    ...
    const createdPatientUsers = await Promise.all(this.generatedPatientUsers.map(async user => await this.userRepository.save((user))));
    const createPatients: Partial<Patient>[] = createdPatientUsers.map(user => ({
      user: user,
    }));

    const createdPatients = await Promise.all(createPatients.map(async patient => await this.patientRepository.save(this.patientRepository.create(patient))));
    this.generatedPatientInsurances.forEach((insurance, index) => {
      insurance.patient = createdPatients[index % createdPatients.length];
    });

    await Promise.all(this.generatedPatientInsurances.map(async insurance => await this.patientInsuranceRepository.save(insurance)));
  }
