...
export class DoctorSeeder implements Seeder {
  private generatedDoctorUsers: Partial<User>[] = Array.from({ length: 50 }, () => {
  ...
  private generatedDoctorEmployeeMap: Map<string, Partial<Employee>> = new Map(this.generatedDoctorUsers.map(user => [user.userName!, {
    startDate: faker.date.past({ years: 5 }),
    department: faker.helpers.arrayElement(
      Object.values(EmployeeDepartment).filter(
        dept => ![EmployeeDepartment.BackOffice, EmployeeDepartment.FrontOffice, EmployeeDepartment.Pharmacy, EmployeeDepartment.Radiology].includes(dept)
      )),
  }]));


  private generatedDoctorScheduleMap: Map<string, Partial<DoctorSchedule>[]> = new Map(
  ...
  async seed() {
    ...
    const createdDoctors = await Promise.all(createdEmployees.map(async employee => {
    ...
    createdDoctors.forEach(doctor => {
      const schedules = this.generatedDoctorScheduleMap.get(doctor.employee.user.userName!);
    ...
    await Promise.all(doctorSchedules.map(async schedule => await this.doctorScheduleRepository.save(this.doctorScheduleRepository.create(schedule))));
  }
