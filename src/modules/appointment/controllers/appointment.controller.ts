...
export class AppointmentController {
  ...
  @Get(':appointmentId')
  async findById(@Param('appointmentId') appointmentId: string): Promise<Appointment> {
  ...
  @RequiredRole(UserRole.User)
  @Post()
  async create(@Body() createAppoinmentDto: CreateAppointmentDto): Promise<string> {
    return this.appointmentService.create(createAppoinmentDto);
  }

  @RequiredRole(UserRole.Staff)
  @Put(':appointmentId')
  async update(@Param('appointmentId') appointmentId: string, @Body() closeAppoinmentDto: CloseAppointmentDto): Promise<void> {
    return this.appointmentService.update(appointmentId, closeAppoinmentDto);
  }

  @RequiredRole(UserRole.User)
  @Put(':appointmentId/reschedule')
  async reschedule(@Param('appointmentId') appointmentId: string, @Body() updateAppoinmentTimeDto: UpdateAppointmentTimeDto): Promise<void> {
    return this.appointmentService.update(appointmentId, updateAppoinmentTimeDto);
  }
